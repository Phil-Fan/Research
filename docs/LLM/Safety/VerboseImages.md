...
![alt](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Research__Papers__assets__VerboseImages.assets__VerboseImages.webp)
...
- VLM 对于同一个输入，输出也可能不同（sampling policies）；但是 NICGSlowDown 优化特定输出 token 的对率

![alt](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Research__Papers__assets__VerboseImages.assets__image-20250804214551732.webp)
...
- $[g_1(x'),g_2(x'), \cdots g_n(x')]$ 表示所有生成的 token 的隐藏状态的拼接矩阵，**增加隐藏状态矩阵的秩**会得到 **更多样化的 token 隐藏状态** 。
...
![alt](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Research__Papers__assets__VerboseImages.assets__image-20250804220529126.webp)
...
![alt](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Research__Papers__assets__VerboseImages.assets__image-20250804220104948.webp)

### Distribution

![alt](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Research__Papers__assets__VerboseImages.assets__image-20250804220116470.webp)

### Chair

![alt](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Research__Papers__assets__VerboseImages.assets__image-20250804220134095.webp)

### ablation Exp

![alt](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Research__Papers__assets__VerboseImages.assets__image-20250804220331424.webp)

### 黑盒攻击

![alt](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Research__Papers__assets__VerboseImages.assets__image-20250804220405115.webp)

### 其他任务

![alt](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Research__Papers__assets__VerboseImages.assets__image-20250804220423481.webp)
...
下载 coco 数据集[COCO - Common Objects in Context](https://cocodataset.org/#download)
...
!!! note "这篇作者其实没有把所有的代码放出来，只放了算法部分的 demo，所以想要复现所有 set 的话，需要花费一定时间。感觉作者不是很厚道啊 xs"
...
1.61 是有 log 函数的，[log — SciPy v1.16.0 手册 - SciPy 科学计算库](https://docs.scipy.org.cn/doc/scipy/reference/generated/scipy.stats.log.html)
...
```python
#提取 mp4 视频里的音频保存为 mp3.py
from moviepy.video.io.VideoFileClip import VideoFileClip
```

### BERT 模型加载

这里因为 autodl 的网络问题，所以我们需要自己下载一下 pretrained 的这个 bert(因为 autodl 已经下载过了相当于，在`/root/autodl-pub/BERT-Pretrain-Modelbert-base-uncased.zip`中)
...
又因为 thinc released 8.3.0 with a depen·dency on numpy 2，如果要使用 numpy=1.*，就需要安装`thinc<8.3.0`
...
原因：在 huggingface_hub 0.26 中移除了如下函数，[原链接](https://github.com/huggingface/huggingface_hub/pull/2579)
